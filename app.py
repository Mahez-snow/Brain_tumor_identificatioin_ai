...
MODEL_URL = (
    "https://huggingface.co/mahez/braintumor/resolve/main/brain_tumor_final.keras"
)
...
with right:
    ...
    if uploaded_file and analyze:
        with st.spinner("Preparing-report(🧬)..."):
            ...
            st.markdown(
                "<div class='status-active'>● ANALYSIS COMPLETE ⌛</div>",
                unsafe_allow_html=True,
            )
            st.markdown(
                f"<div class='prediction-output'>{class_names[idx].upper()}</div>",
                unsafe_allow_html=True,
            )
        ...
        st.info(
            "System awaiting input. Please upload a brain MRI scan to initiate the analysis sequence."
        )
    st.markdown(
        """
